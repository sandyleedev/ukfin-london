...
export default function App() {
  ...
  if (error) {
    return (
      <div className="h-full flex items-center justify-center p-6">
        <div className="bg-card border border-line rounded-lg p-6 max-w-md text-center">
  ...
  return (
    <div className="h-full flex flex-col">
      <Header generatedAt={data.generated_at} adjudicator={data.adjudicator} />

      <main className="flex-1 overflow-auto no-scrollbar p-4 space-y-4">
...
function ClusterDrawer({ cluster, onClose }) {
  return (
    <div className="fixed inset-0 z-20 flex justify-end" onClick={onClose}>
      ...
      <div
        className="relative w-[440px] max-w-[90vw] h-full bg-card border-l border-line shadow-2xl overflow-auto no-scrollbar"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="px-5 py-3 border-b border-line flex items-start justify-between sticky top-0 bg-card">
          <div>
